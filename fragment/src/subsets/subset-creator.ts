...
import { IndexedGeometry, ItemsMap } from './items-map';
...
export class SubsetCreator {
  ...
  constructor(private items: ItemsMap, private subsets: Subsets) {}

  createSubset(config: SubsetConfig, subsetID: string) {
    if (!this.items.map[config.fragment.id]) this.items.generateGeometryIndexMap(config.fragment);
    ...
    if (config.applyBVH) BVH.apply(subset.geometry);
  ...
  private initializeSubset(config: SubsetConfig, subsetID: string) {
    ...
    this.subsets[subsetID] = { ids: new Set<number>(), mesh: model, bvh };
  }
  ...
  private insertNewIndices(
    ...
    const items = this.items.map[config.fragment.id];
