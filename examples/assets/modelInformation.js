import{c5 as E,c6 as P,c7 as A,c8 as q,c9 as U,ca as O,cb as R,cm as _,bS as j,cc as T,cd as L,ce as v,cn as S,co as V,cf as H}from"./pako.esm-9qfSKSXL.js";import{R as M}from"./rendered-faces-DtNZp-Dg.js";const g=new E,W=g.get(P),s=W.create();s.scene=new A(g);s.scene.setup();s.scene.three.background=null;const C=document.getElementById("container");s.renderer=new q(g,C);s.camera=new U(g);s.camera.controls.setLookAt(183,11,-102,27,-52,-11);g.init();const G=g.get(O);G.create(s);const Y="https://thatopen.github.io/engine_fragment/resources/worker.mjs",z=await fetch(Y),X=await z.text(),J=new File([new Blob([X])],"worker.mjs",{type:"text/javascript"}),K=URL.createObjectURL(J),p=new R(K);s.camera.controls.addEventListener("rest",()=>p.update(!0));s.camera.controls.addEventListener("update",()=>p.update());p.models.list.onItemSet.add(({value:e})=>{e.useCamera(s.camera.three),s.scene.three.add(e.object),p.update(!0)});const Q=await fetch("https://thatopen.github.io/engine_fragment/resources/frags/school_arq.frag"),Z=await Q.arrayBuffer(),c=await p.load(Z,{modelId:"example"}),ee={color:new _("gold"),renderedFaces:M.TWO,opacity:1,transparent:!1};let l=null;const te=async()=>{l&&await c.highlight([l],ee)},k=async()=>{l&&await c.resetHighlight([l])};let D=()=>{},$=()=>{};const N=new j;C.addEventListener("click",async e=>{N.x=e.clientX,N.y=e.clientY;const a=await c.raycast({camera:s.camera.three,mouse:N,dom:s.renderer.three.domElement}),m=[];a?(m.push(k()),l=a.localId,D(),m.push(te())):(m.push(k()),l=null,$()),m.push(p.update(!0)),Promise.all(m)});const F=async e=>{if(!l)return null;const[a]=await c.getItemsData([l],{attributesDefault:!e,attributes:e});return a},ae=async()=>{const e=await F(["Name"]),a=e==null?void 0:e.Name;return a&&"value"in a?a.value:null},ne=async()=>{if(!l)return null;const[e]=await c.getItemsData([l],{attributesDefault:!1,attributes:["Name","NominalValue"],relations:{IsDefinedBy:{attributes:!0,relations:!0},DefinesOcurrence:{attributes:!1,relations:!1}}});return e.IsDefinedBy??[]},oe=e=>{const a={};for(const[m,u]of e.entries()){const{Name:b,HasProperties:i}=u;if(!("value"in b&&Array.isArray(i)))continue;const o={};for(const[t,n]of i.entries()){const{Name:r,NominalValue:d}=n;if(!("value"in r&&"value"in d))continue;const y=r.value,w=d.value;y&&w!==void 0&&(o[y]=w)}a[b.value]=o}return a},se=async(e,a=!1)=>{const m=await c.getItemsOfCategory(e),u=(await Promise.all(m.map(o=>o.getLocalId()))).filter(o=>o!==null),i=(await c.getItemsData(u,{attributesDefault:!1,attributes:["Name"]})).map(o=>{const{Name:t}=o;return t&&!Array.isArray(t)?t.value:null}).filter(o=>o);return a?[...new Set(i)]:i},le=async()=>await c.getSpatialStructure(),ie=async()=>{const e=await c.getItemsOfCategory("IFCBUILDINGSTOREY"),a=(await Promise.all(e.map(i=>i.getLocalId()))).filter(i=>i!==null),m=await c.getItemsData(a,{attributesDefault:!1,attributes:["Name"]});let u=null;for(const[i,o]of m.entries())"Name"in o&&"value"in o.Name&&o.Name.value==="01 - Entry Level"&&(u=a[i]);return u===null?null:await c.getItemsChildren([u])};T.init();const re=await c.getCategories(),x=L.create(()=>v`<bim-dropdown name="categories">
    ...
    <bim-panel id="controls-panel" active label="Model Information" class="options-menu">
      ...
      <bim-panel-section label="Selected Item">
        <bim-label style=${S({whiteSpace:"normal",display:l?"none":"unset"})}>💡 Click any element in the viewer to activate the data log options.</bim-label>
        <bim-label ${V(b)} style=${S({whiteSpace:"normal",display:l?"unset":"none"})}></bim-label>
  ...
  `);document.body.append(ce);const f=new H;f.showPanel(2);document.body.append(f.dom);f.dom.style.left="0px";f.dom.style.zIndex="unset";s.renderer.onBeforeUpdate.add(()=>f.begin());s.renderer.onAfterUpdate.add(()=>f.end());
