import{C as M,W as A,S as C,a as U,b as D,G as F}from"./virtual-memory-controller-CWnYvOUm.js";import{a as q,R as v,m}from"./index-CWj6LyOo.js";import{S as B}from"./stats.min-Cj8wREqt.js";import{F as j}from"./index-gTtqA1ko.js";const r=new M,x=r.get(A),t=x.create();t.scene=new C(r);t.scene.setup();t.scene.three.background=null;const _=document.getElementById("container");t.renderer=new U(r,_);t.camera=new D(r);t.camera.controls.setLookAt(58,22,-25,13,0,4.2);r.init();const E=r.get(F);E.create(t);const P="https://thatopen.github.io/engine_fragment/resources/worker.mjs",a=new j(P);t.camera.controls.addEventListener("rest",()=>a.update(!0));a.models.list.onItemSet.add(({value:n})=>{n.useCamera(t.camera.three),t.scene.three.add(n.object),a.update(!0)});const G=async(n,e)=>{const b=await(await fetch(n)).arrayBuffer();await a.load(b,{modelId:e})},O=async n=>{const e=a.models.list.get(n);if(!e)return null;const s=await e.getBuffer(!1);return{name:e.modelId,buffer:s}},L=()=>[...a.models.list.values()].map(s=>s.modelId),y=async(n=L())=>{const e=[];for(const s of n)e.push(a.disposeModel(s));await Promise.all(e)};q.init();const[u,$]=v.create(n=>{const e=L(),s=async({target:o})=>{const d=o.getAttribute("data-name");if(!d)return;const l=`school_${d}`;o.loading=!0,e.includes(l)?await y([l]):await G(`/resources/frags/${l}.frag`,l),o.loading=!1},b=()=>y(),p=async({target:o})=>{const d=o.getAttribute("data-name");if(!d)return;const l=`school_${d}`;o.loading=!0;const w=await O(l);if(w){const{name:R,buffer:I}=w,c=document.createElement("a"),h=new File([I],`${R}.frag`);c.href=URL.createObjectURL(h),c.download=h.name,c.click(),URL.revokeObjectURL(c.href)}o.loading=!1},f=e.some(o=>o.includes("arq")),g=e.some(o=>o.includes("str")),k=f?"Remove Architecture":"Load Architecture",S=g?"Remove Structure":"Load Structure";return m`
  ...
  `);document.body.append(W);const i=new B;i.showPanel(2);document.body.append(i.dom);i.dom.style.left="0px";i.dom.style.zIndex="unset";t.renderer.onBeforeUpdate.add(()=>i.begin());t.renderer.onAfterUpdate.add(()=>i.end());
