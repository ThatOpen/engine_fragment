...
import { CircularSweepData, ProfileData } from "../../src";
...
// prettier-ignore
const cSweepData: CircularSweepData = {
  profilePoints: [
    0, 0, 0, 
    1, 0, 0, 
    1, 1, 0,
...
const update = () => {
  ...
  cSweepData.profilePoints = geometryEngine.getProfilePoints(profileData);
  geometryEngine.getCircularSweep(cSweep.geometry, cSweepData);
};
...
const [panel] = BUI.Component.create<BUI.PanelSection, any>((_) => {
  return BUI.html`
    <bim-panel style="min-width: 25rem;" id="controls-panel" active label="Circular sweep" class="options-menu">
      ...
      <bim-checkbox label="Closed" value=${false} @change=${(e: any) => {
        cSweepData.closed = e.target.value;
        update();
      }}></bim-checkbox>
      ...
      <bim-number-input label="Radius" slider min=0 max=5 step=0.01 value=${1} @change=${(
        ...
        cSweepData.radius = e.target.value;
        update();
      }}></bim-number-input>
