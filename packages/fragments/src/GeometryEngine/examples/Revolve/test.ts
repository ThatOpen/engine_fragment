...
const update = () => {
  geometryEngine.getRevolve(revolve.geometry, revolveData);
};
  ...
  return BUI.html`
    <bim-panel style="min-width: 25rem;" id="controls-panel" active label="Revolve" class="options-menu">
      ...
      <bim-number-input label="Segment count" slider min=3 max=60 step=1 value=${-10} @change=${(
        ...
        revolveData.segmentCount = e.target.value;
        update();
      }}></bim-number-input>
      ...
      <bim-number-input label="Start" slider min=0 max=360 step=1 value=${0} @change=${(
        ...
        revolveData.start = e.target.value;
        update();
      }}></bim-number-input>
      ...
      <bim-number-input label="End" slider min=0 max=360 step=1 value=${180} @change=${(
        ...
        revolveData.end = e.target.value;
        update();
      }}></bim-number-input>
